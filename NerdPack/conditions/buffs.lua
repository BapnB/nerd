...
local function UnitBuffL(target, spell, own)
  for i=1,40 do
    local name, _, _, count, type, duration, expiration, caster, isStealable,
...
-- /dump NeP.DSL.Parse("target.debuff(155722)", "", "")
...
-- /dump NeP.DSL.Parse("target.debuff(Rake).duration", "", "")
-- /dump NeP.DSL.Parse("target.debuff(155722).duration", "", "")
...
-- /dump NeP.DSL.Parse("count.enemies.buffs(155722)", "", "")
NeP.DSL:Register("count.enemies.buffs", function(_,buff)
  ...
  for _, Obj in pairs(NeP.OM:Get("Enemy", true)) do
   if _G.UnitCanAttack(Obj.key, "player")
    and NeP.DSL:Get("buff.any")(Obj.key, buff) then
     n1 = n1 + 1
   end
  end
  return n1
...
-- /dump NeP.DSL.Parse("count.enemies.debuffs(Rake)", "", "")
