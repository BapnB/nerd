...
local logDamage = function(...)
	...
	-- Chat Output for Debugging
 	--if DestGUID == _G.UnitGUID("player") then
--		print(spellID)
--	end
	-- Mixed
...
local logSwing = function(...)
	...
	--if GUID == _G.UnitGUID("player") then
...
local addFailAction = function(...)
	local _,_,_, sourceGUID, _,_,_,_, destName, _,_,_, spellName = ...
	if not spellName then return end
	if sourceGUID == _G.UnitGUID('player') then
		local icon = select(3, _G.GetSpellInfo(spellName))
		NeP.ActionLog:Add('Spell Cast Failed', spellName, icon, destName)
	end
	Data[sourceGUID].interrupted = spellName
end
...
local EVENTS = {
	...
	['SPELL_INTERRUPT'] = addFailAction,
	['SPELL_CAST_FAILED'] = addFailAction
}
...
function NeP.CombatTracker.Interrupted(_, unit)
  local GUID = _G.UnitGUID(unit)
  if Data[GUID] then
    return Data[GUID].interrupted
  end
end
