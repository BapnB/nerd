...
local empowered = _G.C_AzeriteEmpoweredItem.IsAzeriteEmpoweredItem
local selected = _G.C_AzeriteEmpoweredItem.IsPowerSelected
local tierInfo = _G.C_AzeriteEmpoweredItem.GetAllTierInfo
local PowerInfo = _G.C_AzeriteEmpoweredItem.GetPowerInfo
...
local function AzeriteScan()
	_G.wipe(Azerite)
	if not _G.IsEquippedItem(158075) then return end
	for i = 1, 5, 2 do
		if _G.IsEquippedItemType(EquipType[i]) then
			local item = _G.ItemLocation:CreateFromEquipmentSlot(i)
				...
				for _, v in pairs(info) do
					for j = 1, #v.azeritePowerIDs do
						...
						and selected(item, v.azeritePowerIDs[j]) then
							local Name = _G.GetSpellInfo(PowerInfo(v.azeritePowerIDs[j]).spellID):lower()
