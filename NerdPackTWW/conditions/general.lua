...
NeP.DSL:Register('timeout', function(_, args)
  ...
  if not tostring(name) or not tonumber(time) then print("--USAGE: timeout(TIMER_NAME, SECONDS)") return end
   if not TimeOutTable[name] then TimeOutTable[name] = true
    C_Timer.After(tonumber(time), function() TimeOutTable[name] = nil end) 
	 return true
   end
...
NeP.DSL:Register('targettimeout', function(target, args)
  ...
  local name, time = _G.strsplit(',', args, 2)
  if not tostring(name) or not tonumber(time) then print("--USAGE: UNIT.targettimeout(TIMER_NAME, SECONDS)") return end
   if not TimeOutUnitTable[target..name] then TimeOutUnitTable[target..name] = true
    C_Timer.After(tonumber(time), function() TimeOutUnitTable[target..name] = nil end)
	 return true
   end
