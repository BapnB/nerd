...
NeP.DSL:Register('lastcast', function(Unit, Spell)
  ...
  local Spell = _G.GetSpellInfo(Spell)
  -- if Unit is player, returns last parser execute.
...
NeP.DSL:Register("lastcast.interrupted", function(_, Spell)
  local Spell = _G.GetSpellInfo(Spell)
  local Interrupted = NeP.CombatTracker:Interrupted("player")
  local LastCast = NeP.CombatTracker:LastCast("player")
  return Interrupted == Spell, Interrupted and LastCast == Spell, LastCast
end)
